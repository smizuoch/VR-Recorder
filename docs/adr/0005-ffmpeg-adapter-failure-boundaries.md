...
隔離した`FfmpegAacPacketEncoder`は公式8.1.2のnative `aac` encoderだけを選び、AAC-LC／48 kHz／stereo／192 kbps／FLTP／global headerを固定する。open後に`frame_size=1024`、`initial_padding=1024`、delayとsmall-last-frame capability、AudioSpecificConfig `11 90 56 e5 00`を読み戻せないbuildは拒否する。descriptorのextradataはcontextからdeep copyし、encoder破棄後も所有値として保持する。

既存mixerのpacked Float32 stereoは、同rateの`libswresample`でplanar FLTPへ変換し、`AVAudioFifo`へ積む。active中は1024 sample単位だけを送信し、`Finish`でresamplerをNULL input／0 input countによりflushした後、残り1–1023 sampleをzero paddingせずsmall last frameとして1回送り、最後にNULL frameを送ってEOFまでdrainする。入力shape、finite値、48 kHz frame連続性、microsecond rescale可能範囲はcodec mutation前に検証する。

encoder operationは単一mutexで線形化し、失敗した呼出し内で先に生成されたpacketを部分公開しない。実8.1.2で0／1／1023／1024／1025 input sampleのpacket列、負1024-sample priming、短い末尾duration、side data 0件を固定する。これはAAC単体の契約証拠であり、MOVへmuxした後の末尾presentation sample数、decode、playback、production compositionを証明しない。
...
この保証はcallback stack上でfull recording sessionがpeer workerを同期Joinすることまでは含まない。peer workerが同じoperation gate待ちの場合はcross-stream循環になり得るため、production composition前にcallback dispatchまたはpeer Joinをcallback stack外へdeferするRedを追加する。recursive Submit／FinishはPort契約外である。
...
- codec parametersのexact bitrate／`frame_size`／`initial_padding`、extradata padding、fragment options、interleaved ownership
...
- common batch Portはdrift-aware pipelineの迂回、batch内invalid prefix mutation、A/V batch interleave、Finish先行、Abort後logical commitをportable testで防ぐ。ただしfull worker graphのcallback→peer Joinはproduction composition gateとして残る。
...
- FFmpeg 8.1.2 MOV bitrate metadata fallback: <https://github.com/FFmpeg/FFmpeg/blob/n8.1.2/libavformat/movenc.c#L717-L800>
...
- FFmpeg 8.1.2 audio conversion and flush contract: <https://github.com/FFmpeg/FFmpeg/blob/n8.1.2/libswresample/swresample.h>
- FFmpeg 8.1.2 audio FIFO contract: <https://github.com/FFmpeg/FFmpeg/blob/n8.1.2/libavutil/audio_fifo.h>
