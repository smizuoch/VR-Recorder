...
| audio/video encoding worker | thread生成OOM／internal／non-joinable、生成成功／失敗中Abort、Abort-before-Start、graceful Finish中Abort、in-flight WriteがAbort後に成功／失敗復帰 | 共通factoryのpublicationをJoinと直列化し、Abortを生成結果／Stopped／Faultedより優先する。pump成功はworker側counter／first-packetへterminalと同じlockでcommitし、Abort後に成功復帰したWriteのpacket／frame／latencyを公開しない。生成失敗winnerでは未起動source／clock／sinkを変更せず、Abort winnerではwrite／event／統計を抑止してcleanupをexactly onceにする | `audio_encoding_worker_tests.cpp`, `video_encoding_worker_tests.cpp` |
