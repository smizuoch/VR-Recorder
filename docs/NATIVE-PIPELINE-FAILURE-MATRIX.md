...
|---|---|---|---|
| P0 | 実FFmpeg/MF encoder／muxer port | 実AAC Portで未完了のcontext生成、Float32変換、FIFO／frame sizing、SkipSamples／discard padding再構築、zero-size／side-data-only packetと`DISCARD`等flagのencoder別実測。H.264 context／frame adapter、global header、codecparの`frame_size`／`initial_padding`、extradata padding、実libavformat Port、header後actual time-base readback、fragment option、interleaved ownership、trailer／close error | 実AAC Port単体だけをproduction Greenと数えず、MOV edit list／decode後presentation 0と末尾paddingをffprobe／playbackで証明、packet漏れ・二重送信なし、Finish後frame拒否、header未成立file／失敗fileをpublishしない |
