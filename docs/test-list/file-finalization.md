...
- [ ] 最終rename成功前にSavedを発行しない
- [ ] 最終fileの再検証成功後だけSavedを発行する
- [ ] MP4検証失敗時はSavedを発行せずrecoveryへ隔離する
...
- [ ] Do not publish Saved before the final rename succeeds
- [ ] Publish Saved only after the final file passes reopen validation
- [ ] Publish no Saved event and quarantine to recovery when MP4 validation fails
