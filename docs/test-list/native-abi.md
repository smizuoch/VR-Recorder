...
- [x] Windows production FFmpeg evidence schema v2をMSVC 19.44.35228／Windows SDK 10.0.26100.0、source archive／build recipe、exact 4 DLL＋4 import libraryの実length／SHA-256へ結合し、toolchain drift、missing、tamperをconfigure時に拒否する
...
- [x] Bind Windows production FFmpeg evidence schema v2 to MSVC 19.44.35228/Windows SDK 10.0.26100.0, the source archive/build recipe, and actual lengths/SHA-256 values for the exact four DLLs plus four import libraries, rejecting toolchain drift, missing files, and tampering at configure time
