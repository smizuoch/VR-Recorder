...
- post-publish sealerはexisting canonical root、normalized root-relative GUI AMD64 PE entrypoint、全fileのcanonical path／length／SHA-256／kind、Windows case-fold duplicate、reparse／ADS semanticsを再scanする。propsに固定した各runtimeのhash／length／kindとpublish結果を照合し、stagingとpublishを同時に改変した場合も拒否する。final publish rootのLegal BundleをpropsのID／manifest hashで認証した後だけinventory／EXE／Legal identityを発行する。全publish PEのimport再照合とsource revision／product versionの追加は次の単位に残る。
- official publish scriptはstaging CLIが返したexact propsだけを受け取り、別digest directory差替えを拒否する。publish後のsealer CLI接続と決定的identity JSON出力は次の単位に残る。
...
| P0 | approved runtime staging | manifest v2、full-production closure、PE admission／import closure、repository graph builder、CLI、two-phase publish、post-publish runtime／Legal sealは自動test Green。actual canonical registryは独立Legal承認待ちでfail-closed。source revision／product versionとsealer CLI出力が未完了 | reproducible full Release payload |
