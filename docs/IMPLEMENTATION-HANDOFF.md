...
- 更新日: 2026-07-14
- 対象branch: `main`
- 実装基準commit: `179ce3b`（AAC compositionとfailure-point追試。本文更新直前の論理commit）
...
1. Linuxのexact contract-test SDKで、production AAC attach contractとnonzero AAC packet→実MOVをRedから完成させる。ただしactual artifact admission前にmain DLLへproduction linkを着地させない。
2. strict runtime staging manifest／transactional copy／factory-evidence consumptionをRedから実装する。
3. accepted済みの[`ADR-0006`](adr/0006-encoder-identity-preheader-and-production-factory-contract.md)を変更せず、actual Windows FFmpeg／vendor SDK／ffprobe／decode oracleの再現build、hash、Legal admissionを揃え、AACをproduction media variantへ接続する。
...
`c794817`と直後のfailure-point追試時点で次を確認した。

- portable CMake full build: 成功
- portable Linux native CTest: 44/44成功
- exact FFmpeg 8.1.2 contract-test SDK構成のfull build: 成功
- real FFmpegを含むLinux native CTest: 49/49成功
- 新規`ffmpeg_aac_audio_pipeline`は、real AAC encoderで最初の1024-frame encodeが0 packetでも成功を数えず、Stop時のFinish packetだけを同期submissionへ渡すことを確認
- `git diff --check`: 成功
- GCC TSan: test process開始前にhost固有の`unexpected memory mapping`で停止。TSan成功は主張しない

managed全件とcoverageは今回再採取していない。前回値は[`VALIDATION-REPORT.md`](../VALIDATION-REPORT.md)にあるが、古い数値を現在値として扱わない。Windows MSVC production-link、実MOV demux／decode、Windows／GPU／SteamVR実機は未検証である。
...
`ffmpeg_aac_audio_pipeline_tests.cpp`はreal AAC packetがcompositionから同期`RecordingSubmission` fakeへ到達し、Stop時を含めfakeが`Written`として受理したpacket数とpipeline統計が一致することまで検証する。これは実muxのwrite件数を検証した証拠ではない。一方、`ffmpeg_aac_to_fragmented_mp4_integration_tests.cpp`はまだAAC descriptorとzero-packetのMOV headerだけであり、real AAC packetを実MOVへ書いてdemux／decodeした証拠ではない。ここをWork package A／Cで混同しない。
...
|---|---|---|---|
| P0 | production AAC接続 | real encoder→sink→session compositionとStop flushはGreen。main DLL link／実MOV demuxは未完了 | 実録画backend |
...
1. production media variant専用のCMake attach contractをRedにする。必要sourceは`ffmpeg_aac_audio_pipeline.cpp`、`ffmpeg_aac_packet_encoder.cpp`、`ffmpeg_libavcodec_encoder_port.cpp`だけで、既収録のconfig／state machine／sink／session／worker／pumpを重複追加しない。
2. production linkは`FFmpeg::avcodec`、`FFmpeg::avutil`、`FFmpeg::swresample`のexact imported targetだけを許し、`FFmpegContractTest::*`、ambient `find_package`、unversioned libraryへのfallbackを禁止する。placeholder media variantへFFmpeg importを混ぜない。
3. このlinkをmain DLLへ追加するとComplianceがactual runtime artifact、source archive、build recipe、approvalを要求する。Work package 0のWindows SDK evidenceが揃うまで仮hashで通さず、production linkを未完了gateのまま保つ。
4. `ffmpeg_aac_to_fragmented_mp4_integration_tests.cpp`へnonzero AAC packetとFinish packetを書き、`mdat`／packet countを確認する。別processのpinned oracleでAAC-LC、48 kHz、stereo、packet count > 0、bitrate metadataをdemux検証する。
5. Windows MSVC＋actual pinned SDKでproduction DLLをlinkし、test executable横へ`avcodec-62.dll`、`avutil-60.dll`、`swresample-6.dll`を明示配置する。
6. `production_media_backend.cpp`が後続のWork package Cから所有できるAAC composition factory seamを公開し、actual pinned SDKを使うproduction variantのfactory smoke testを通す。ここまでをWork Aの「production AAC component Green」とし、capture／`PreHeaderCoordinator`／muxへの接続はWork package Cの完了条件にする。
...
> `docs/IMPLEMENTATION-HANDOFF.md`とADR-0006を正として、まずWork package A残りのproduction AAC attach contractとnonzero AAC packet→実MOV testをLinuxのexact FFmpeg SDKでRedから進めて。main DLLへのlinkはactual Windows artifact／Legal admissionまで保留し、次にstrict runtime staging manifest、actual Windows SDK、Work package Bのportable Annex B／SPS crop／avcCへ進み、各論理単位でreal test、full regression、commitまで行って。
