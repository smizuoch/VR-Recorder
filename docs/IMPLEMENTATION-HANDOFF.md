...
- 更新日: 2026-07-14
- 対象branch: `main`
- 実装基準commit: `cf07e1b`（strict Windows runtime stagingのtop-level fail-closed境界）
...
1. production SDKと同じFFmpeg 8.1.2 source identityからtest-onlyの別process demux／decode oracleを再現buildし、AACのcodec／metadata／packet timelineとMP4上のpresented sample数を固定する。oracleをproduction payloadへ混ぜない。
2. accepted済みの[`ADR-0006`](adr/0006-encoder-identity-preheader-and-production-factory-contract.md)を変更せず、actual Windows FFmpeg／vendor SDK／ffprobe／decode oracleのbuild evidence、hash、Legal admissionを揃え、AACをproduction media variantへ接続する。
3. actual artifact identityが確定した時点で、staging manifest v2のfull-production profile／declared length／Legal anchor、repository由来`ApprovedReleaseGraph` builder、外部staging CLIをRedから追加する。自己申告のapproved boolや仮hashで現v1をGreenにしない。
...
13. full-production staging profile、two-phase Release publish、post-publish inventory sealerをactual artifactで通す。
14. 全機能を含むunpackaged directoryを再生成・再identity化し、Windows／GPU／VRChat／SteamVR／HMDの最終Hardware Validation Payloadとして合格させる。
15. その合格payloadだけを別projectでMSIXへ包む。
...
| P0 | approved runtime staging | strict manifest／path、ApprovedGraph owner、native registry、factory evidence、copy／再hash／exact inventory、immutable digest publish、props-only Release importはGreen。full-production profile／declared length／Legal anchor、actual artifact、repository graph builder、CLI、post-publish sealerは未完了 | reproducible full Release payload |
...
5. actual artifact一覧が確定したらschema v2をRedから追加し、`profile=full-production-hardware-validation-v1`、`runtimeIdentifier=win-x64`、declared `length`、Legal Bundle ID／manifest SHA、required role closureを固定する。v1を暗黙拡張せずversionを上げる。
6. PE admission readerをRedから追加し、native DLL／EXEについてDOS／PE signature、PE32+、AMD64 machine、想定subsystem、entrypoint有無、import tableを実bytesから検証する。必須importがpayload外や許可していないambient DLLへ解決される構成を拒否し、ASCII `.dll` fixtureをGreenにしない。
7. canonical repository evidenceから`NormalizedComponentGraph`を構築し、`ReleaseEligibilityGate`だけが`ApprovedReleaseGraph`を発行するtrust-sourceを実装する。現registryにない`RequestedBy`相当をschema／review workflowへ追加し、自己申告approvalを受け取らない。
8. stagingとpublishを別process invocationにするCLI／scriptを実装する。stagerが返したexact props pathとcontent-address identityを新規`dotnet publish`へ渡し、hand-written props、staging後の改変、別digest directoryへの差替えを拒否する。publish後にmanaged／self-contained .NET／Legalを含む全fileを再hashしてruntime manifestと照合する。MSBuildはevaluation後に生成されたImportを同じbuildへ反映しないため、1 invocationへ押し込まない。
...
2. oracleを別processで起動し、AAC-LC、48 kHz、stereo、packet count、bitrate metadata、PTS/DTS、raw decode可能性、MP4 box上のpresented sample数を機械可読結果から検証する。timeout、nonzero exit、signal、malformed／missing field、version drift、余分なstreamも個別にRedにする。
3. 3秒／48 kHz入力に対してMP4上の`presented_sample_count == 144'000`をtail gateにする。raw AAC decoder出力はAAC frame単位のpaddingを含み得るため、raw decoded sample数や3秒packet-end assertionをpresentation長の代用にしない。FFmpeg 8.1.2のfragmented MOVでは最終`trun` durationとAAC edit listをoracleで読む。
...
> `docs/IMPLEMENTATION-HANDOFF.md`とADR-0006を正として、strict runtime staging foundationは`cf07e1b`までをbaselineにして作り直さないで。まず同じFFmpeg 8.1.2 source identityから別processのtest-only demux／decode oracleを作り、3秒／48 kHz inputに対するMP4上のpresented sample完全一致をRedにしてAAC末尾paddingを閉じて。actual Windows artifact／Legal admission前にmain DLLへFFmpegをlinkしないで。artifact identity確定後にmanifest v2のfull-production profile／length／Legal anchor、repository-derived ApprovedGraph builder、staging CLI、post-publish sealerをRedから追加し、その後Work package Bのportable Annex B／SPS crop／avcCへ進めて。各論理単位でreal test、full regression、commitまで行って。
