...
| P0 | approved runtime staging | manifest v2、full-production closure、PE admission／import closure、repository graph builder、CLI、two-phase publish、post-publish runtime／Legal／build identity sealは自動test Green。actual canonical registryは独立Legal承認待ちでfail-closed。sealer CLI出力と全publish PE import再照合が未完了 | reproducible full Release payload |
