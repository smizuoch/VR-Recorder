...
- 現在の判定: release-payload identity／Hardware Validation evidence contract／settings schema validation、coverage metrics収集、SingleFileFit／ExactFollowSegmentsの自動化経路は実装済み。coverage／mutationの数値閾値はrelease条件ではない。UI Automationとproduction機能残を実装中で、配布製品でもrelease候補でもない
