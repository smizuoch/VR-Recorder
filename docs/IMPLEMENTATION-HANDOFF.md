...
- lifecycle／textureと分離したnative overlay event Port。overlay create transaction内で1024×512 pointer inputをconfigureし、top-left pixelへ変換済みのMove／ButtonDown／ButtonUpだけをbutton／bounds検証後に返す。configure失敗はDestroy rollback、不正runtime eventはゼロ化してfail-closedにする
