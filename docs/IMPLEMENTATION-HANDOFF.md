...
- 更新日: 2026-07-18
- 対象branch: `main`
- 実装基準commit: `77eef94`（ExactFollowSegmentsの開始・安定geometry変更・part002以降への自動移行）
- 現在の判定: FFmpeg 8.1.2 hardware SDK、D3D11→NVENC→MP4、AMF／QSV共通構造、Auto／software fallback、OpenVR／Wrist／telemetry、Windows UI Automation／first-run、およびfull-production payload closureは実装済み。未承認runtimeを含むDebug self-contained publishはbring-up用draftとして検証済みだが、独立Legal承認済みgraphとclean source revisionがないためRelease identityは未発行。配布製品でもrelease候補でもない
- 配布方針: unpackaged self-contained `win-x64` payloadで実機検証し、同一payloadの合格後だけMicrosoft Store MSIX候補へ進める

## 1. 最初に読む結論

desktop production録画、production OpenVR overlay、first-run setup 7／8のproduction route、PICO microphoneのdirect captureとMicOnly production mux／oracleは完了した。SingleFileFitの録画中layout更新と、ExactFollowSegmentsの500 ms安定判定からpart002以降への自動移行も自動testでGreenになった。release payload identity、versioned Hardware Validation report／required matrix validator、settings schema、native・managed line／branch metrics収集も自動化した。coverage／mutationは参考指標であり、最低率を満たすためだけの作業は行わない。実機検証は最終Hardware Validationへまとめる。UI Automationとproduction機能残を閉じ、payloadをfreezeしてからWindows／GPU／VRChat／SteamVR／HMDを同一identityで一括検証する。独立Legal承認は外部gateとしてfail-closedのまま維持し、自動化可能な実装と直列化しない。

2026-07-18 hardware-production checkpointでは、NVENC／AMF／QSV入りFFmpeg 8.1.2 MSVC x64 shared SDKを一度だけ構築し、persistent D3D11 NV12 texture poolから実NVENCへ30 frameを渡した。AACとfragmented MP4へmuxした1秒録画は別processのpinned oracleでvideo 30 frame／audio 48 kHzをdecode確認した。AMFはD3D11 surface、QSVはD3D11 derived deviceを使う同一session構造へ接続し、Autoの実NVENC probeとmanaged clean software fallbackをGreenにした。production OpenVR overlay／Wrist描画／controller／haptics／move／pin／telemetry、実WPFを使うfirst-run／SaveFileDialog UI Automationも接続済みである。full-production native DLLは4 familyすべてproductionとしてbuildし、FFmpeg 4 DLL、`libvpl.dll`、ffprobe、OpenVR、manifest群、app-local MSVC CRT 4 DLLを含むself-contained `win-x64` draftのload／oracle起動まで確認した。このdraftはLegal admissionもpayload identity発行も通しておらず、freeze済み成果物ではない。

freezeの残件は外部gateである。canonical registryのruntime admission対象はFFmpeg、JsonSchema.Net、libvpl、Microsoft Visual C++ Runtime、OpenVR、Spout2のexact 6 componentで、すべて`pending-independent-review`のままである。MSVC runtimeはMicrosoft署名済みVC143 x64 installer package identity、公式redistribution list／Runtime license全文、実4 DLLのlength／SHA-256をcandidate evidenceへ固定し、development closureだけを許可してReleaseの`nativeArtifacts`登録は行っていない。残るのは実在するrequester／ticket／independent reviewerによる6 componentの承認metadata、承認graphから生成する認証済みLegal Bundle／`LEGAL-MANIFEST.sha256`、およびfreeze対象application source revisionを確定するclean commitである。設計上、full-production manifest v2はrepository-derived ApprovedGraphとauthenticated Legal anchorなしではRelease publishを必ず拒否するため、placeholder承認や手動copyでidentityを発行してはならない。Store候補前の最終Legal承認とは別に、runtime component admissionはpayload freezeの入力として先に必要である。

2026-07-16 checkpointでは、公式Spout demo senderの640×360 BGRA8 frameとdefault render endpointのWASAPI loopbackを、production `vrrecorder_native.dll`のSpout2→D3D11 NV12→software `h264_mf`およびAAC→fragmented MP4経路へ通した。共有D3D11 immediate contextには`ID3D11Multithread`保護を有効化し、CFR frame PTSはcodec tickのままlibavcodecへ渡す。実録画はvideo 117 packet／3.900秒／30 fps、audio 193 packet／4.117秒／48 kHz stereo、container 4.133秒となり、別rootのpinned `ffprobe`でpending fileを検証してからだけfinal `.mp4`へrenameされた。PICO `default-capture`のdirect production WASAPI HILでは、session前packetとshort discontinuity後のdevice-position gapを修正し、3秒／48 kHz captureを500 ms間隔で10回連続成功させた。さらに同じPICO endpointをMicOnlyでfull production sessionへ接続し、Spout映像110 H.264 packetとmicrophone音声179 AAC packetをmuxした。pinned oracleはvideo 110 frame、audio 178 decoded frame／182,272 sample、audio presentation 0.000～3.797秒を確認し、検証後だけfinal `.mp4`へ公開した。独立Legal review／canonical registry admission、VRChat sender、device change／privacy、actual NVENC／AMF／QSV、VRChat解像度切替を使うExactFollowSegments HIL、controller HIL、最終payload／MSIXは未完了であり、Release admissionは引き続きfail-closedである。古いcheckpointの「production media未接続」「実Spout／WASAPI未完了」という記述より本段落、2.7節、2.8節を優先する。

推奨順は次のとおり。

1. coverage／mutationの数値追求は行わず、UI Automationと実機を要しないhardware encoder／OpenVR／telemetryのproduction機能残を閉じる。
2. completed staging／identity gateで全機能を含むunpackaged directoryをfreezeする。
3. freeze済みの同一identityに対してWindows／GPU／VRChat／audio／SteamVR／HMDの最終実機検証を一括実行する。
4. 実在するticket／requester／independent reviewerからLegal approvalを取得する。
5. LegalとHardware Validationに合格した同一payloadだけを別projectでMSIXへ包む。
...
- Hardware Validation report schema v1とreaderはpayload identity document hash、run ID／runner ID／UTC timestamp、Windows build、GPU／driver、encoder mode／API／実名、SteamVR／HMD／左右controller、case status、artifact path／length／hash／kindだけを受理する。unknown／duplicate field、unknown status、duplicate run／case／artifactを拒否し、serialized `Passed` propertyはschemaに存在しない。
- evidence validatorはidentity document hash一致、全case `pass`、required case closure、Windows 10 22H2／11、NVIDIA/NVENC・AMD/AMF・Intel/QSV、software Media Foundation、接続済みSteamVR／HMD／左右controller、artifact rootのexact path／length／hash／regular-file semanticsからだけinternal evidenceを発行する。`fail`／`skip`、missing／tamper／extra artifact、matrix欠落では発行しない。正本case一覧は[`windows-hardware-validation-matrix-v1.json`](test-list/windows-hardware-validation-matrix-v1.json)へ固定した。
...
### 2.8 2026-07-18 ExactFollowSegments automated checkpoint

録画中geometry変更の自動制御経路を次の論理commitで閉じた。

| Commit | 完了した単位 |
|---|---|
| `e46e31d` | Spoutの幅／高さ／pixel format／surface generation変更frameを隔離し、同じsignatureがsource timestampで500 ms以上継続した場合だけ1回通知 |
| `ec4e3b1` | 48-byte event ABIを維持したkind 14のstable geometry通知とmanaged型付きcallback |
| `640f0e9` | Exact partの無padding layoutと、encoder／audio routing／source identityを維持する次part予約 |
| `e4007e8` | native layout更新成功のacknowledgementまで変更frameをquarantine |
| `ff8d869` | SingleFileFitのin-place更新と、同じrecording handleでExact part002／part003へ移行するruntime engine |
| `77eef94` | ExactFollowSegmentsの初期part開始とIT-009開始契約 |

安定前に元geometryへ戻る場合と別signatureへ変わる場合は候補をresetする。SingleFileFitはlayout更新成功後だけcaptureを再開する。ExactFollowSegmentsは現在partを停止して次partを開き、最初のvideo packetがmuxされた後だけactive sessionを切り替えて前partをfinalizeする。各partは入力と同じ偶数寸法を持ち、暗黙のpadで厳密一致を壊さない。奇数寸法はfail-closedで拒否する。

focused commit gateはnative CTest 3/3、managed対象49/49 Greenだった。これはnative pump／ABI／managed orchestrationの自動制御証拠であり、Legalでfail-closed中のactual production factory再link、実VRChat sender、生成された実fMP4 partのoracle decode、actual GPU encoderとの組合せ、HMD HILの成功は主張しない。Windows Application Controlは再buildしたunsigned managed test DLLを許可しなかったため無効化せず、managed gateは隔離したWSL artifact pathで実行した。
...
- 500 ms stable geometry event、SingleFileFitのacknowledged layout更新、ExactFollowSegmentsのpart001開始とpart002以降への同一handle rollover
...
| P1 | encoder fallback／part rollover | failure前part保持、hardware failure後のsoftware rollover、hardware開始失敗時のclean software file retryに加え、500 ms stable geometry、SingleFileFit更新ack、Exact part002／part003 rolloverは自動test Green。actual NVENC／AMF／QSV failureと実VRChat解像度切替のHILは最終batchへ保留 | 基本設計§4.1／§11.2の実機適合 |
| P1 | unpackaged hardware payload | canonical identity発行／strict再読込、versioned report schema／reader、required matrix／artifact実bytes validatorはGreen。実report収集は最終HIL batchへ保留 | 実機証拠の収集 |
...
基本設計§4.1／§11.2の自動制御contractはGreenである。録画開始前hardware失敗のclean software retry、first packet後failureのpart保持とsoftware rollover、安定geometry変更のSingleFileFit／ExactFollowSegments分岐を作り直さない。残るgateはactual NVENC／AMF／QSV failureと実VRChat sender geometry changeから生成した各partを、freeze済みpayloadでoracle検証するHILである。
...
- fallback／part rolloverの自動制御contractがGreenで、actual GPU／VRChat HILが同一freeze済みpayloadで完了する。
- failed outputを公開しない。
...
> `docs/IMPLEMENTATION-HANDOFF.md`の1節、2.5節、2.8節を正として、`77eef94`までを実装baselineにして再開して。FFmpeg／Spout2／D3D11とdefault-render／PICO MicOnly WASAPIのdesktop production録画、pending oracle検証、final publication、SingleFileFit／ExactFollowSegmentsの自動化経路は作り直さないで。actual hardware encoder、OpenVR binding／production telemetry、Windows UI Automation、payload freezeを実機不要の範囲までRed→最小Greenで閉じ、その後に同一identityでVRChat sender、audio default切替／抜去／privacy、resolution rollover、GPU、SteamVR／HMD／controllerを一括HILして。Fast loopは変更対象だけ、commit gateは同subsystem、full regressionは縦切りの最後に1回とし、coverage／mutationの数値目的テストは追加せず、各論理単位で必ずcommitして。
