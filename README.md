...
# 無期限停止中です。サンクコストを回収できないと発覚したためです。
...
このリポジトリは現在**t-wada式TDDで実装中**です。録画／camera／storage lifecycle、versioned native ABIとmanaged bridge、WPF host、SteamVR Input Action、tray、認証済みLegal UI／保存先mirror、production composition、event-driven WASAPI captureからencoder入力Portまでの境界を含みます。ただし実Spout2／D3D11、承認済みencoder／muxer adapter、OpenVR overlay backendと承認済み配布物は未完了であり、まだ録画可能な配布製品ではありません。

<!-- readme-parity: main-design-requirements -->
### 主な設計要件

- Windows 11 x64正式対応、Windows 10 22H2 x64互換対応
- VRChat Camera OSCとSpout2の自動ON/OFF
- 縦横・解像度の自動検出とアスペクト比を保った録画
- 30 fps初期値、設定モデルは30～120 fpsを保持
- デスクトップ音声＋マイクのmix、Mic OFF、全音声mute
- NVENC／AMF／QSV、利用不能時はsoftware encoderへfallback
- self timerと自動停止
- SteamVR手首UIのREC／STOP／Mic操作、経過時間、NO SIGNAL
- t-wada式TDD。line／branch coverageとmutation scoreは品質傾向を見る参考指標として記録し、数値だけをmerge／release条件にしない
...
<!-- readme-parity: third-party-licensing-and-rights -->
### 第三者ライセンスと権利保護
...
設計版0.3に基づく実装進行中です。Red–Green–Refactorの履歴を逐次commitし、lock fileとcandidate法務台帳を自動照合しています。現在の自動検証値と未達項目は[検証報告](VALIDATION-REPORT.md)に固定しています。Windows実機、実media／OpenVR backend、VRChat／SteamVR、GPU／HMD、最終Legal Bundle、独立法務reviewを含むrelease gateは未完了です。
...
This repository is **under implementation with t-wada-style TDD**. It includes recording, camera, and storage lifecycles; a versioned native ABI and managed bridges; the WPF host; SteamVR Input Action support; tray controls; authenticated Legal UI and output mirroring; the production composition root; and boundaries from event-driven WASAPI capture through an encoder-input port. Real Spout2/D3D11, approved encoder/muxer adapters, the OpenVR-overlay backend, and approved distribution inputs remain unfinished, so this is not yet a recording-capable distributable product.
...
- 30 fps by default, with a settings model that supports 30–120 fps
...
- t-wada-style TDD; line/branch coverage and mutation scores are recorded as quality-trend indicators, not numeric merge or release gates
...
Implementation is in progress against design version 0.3. Red–Green–Refactor history is committed incrementally, and lock files are checked against the candidate legal registry. Current automated evidence and unmet gates are pinned in the [validation report](VALIDATION-REPORT.md). Release gates covering real media and OpenVR backends, Windows hardware, VRChat and SteamVR, GPUs and HMDs, the final Legal Bundle, and independent legal review remain incomplete.
