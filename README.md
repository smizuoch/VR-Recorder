...
このリポジトリは現在**t-wada式TDDで実装中**です。Recording Domain、開始use case、NuGet candidate法務ゲートの初期実装を含みますが、完成した録画アプリケーションや配布可能な実行ファイルではありません。
...
設計版0.3に基づく実装進行中です。Red–Green–Refactorの履歴を逐次commitし、lock fileとcandidate法務台帳を自動照合しています。Windows実機、VRChat／SteamVR、GPU／HMD、最終Legal Bundle、独立法務reviewを含むrelease gateは未完了です。
...
This repository is now **under implementation with t-wada-style TDD**. It includes initial Recording Domain, start-use-case, and NuGet candidate legal-gate code, but it is not yet a complete recorder or a distributable executable.
...
Implementation is in progress against design version 0.3. Red–Green–Refactor history is committed incrementally, and lock files are checked against the candidate legal registry. Release gates covering Windows hardware, VRChat and SteamVR, GPUs and HMDs, the final Legal Bundle, and independent legal review remain incomplete.
