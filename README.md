...
このリポジトリは現在**t-wada式TDDで実装中**です。録画／camera／storage lifecycle、versioned native ABIとmanaged bridge、WPF host、SteamVR Input Action、tray、認証済みLegal UI／保存先mirror、production composition、event-driven WASAPI capture境界を含みます。ただし実Spout2／D3D11、WASAPIからencoderへのsession統合、encoder／muxer／OpenVR overlay backendと承認済み配布物は未完了であり、まだ録画可能な配布製品ではありません。
