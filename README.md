...
このリポジトリは現在**t-wada式TDDで実装中**です。録画／camera／storage lifecycle、versioned native ABIとmanaged bridge、WPF host、SteamVR Input Action、tray、認証済みLegal UI／保存先mirror、production compositionを含みます。ただし実Spout2／D3D11／WASAPI／encoder／muxer／OpenVR overlay backendと承認済み配布物は未完了であり、まだ録画可能な配布製品ではありません。
...
This repository is **under implementation with t-wada-style TDD**. It includes recording, camera, and storage lifecycles; a versioned native ABI and managed bridges; the WPF host; SteamVR Input Action support; tray controls; authenticated Legal UI and output mirroring; and the production composition root. Real Spout2, D3D11, WASAPI, encoder, muxer, and OpenVR-overlay backends and approved distribution inputs remain unfinished, so this is not yet a recording-capable distributable product.
