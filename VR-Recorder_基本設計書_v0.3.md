...
10. t-wada式のRed–Green–Refactorを採用し、結合テスト実行だけで第一者コードの行・分岐カバレッジ90%以上をマージ条件とする。
...
| NFR-010 | テスト | 結合テスト単独実行で第一者コードの行・分岐カバレッジ各90%以上 |
...
```text
Managed line coverage     >= 90%
Managed branch coverage   >= 90%
Native line coverage      >= 90%
Native branch coverage    >= 90%
Public native ABI contract scenarios = 100%
Mutation score            >= 75%
```

除外追加はADRとレビュー承認を必須にする。全体平均で隠さず、主要assemblyごとに90%を要求する。
...
| 90%結合coverageの形骸化 | テスト価値低下 | branch gate、mutation score、公開契約100% |
