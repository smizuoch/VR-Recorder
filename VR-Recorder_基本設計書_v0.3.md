...
- 改訂内容: Material Symbols採用、Apache-2.0自動通知、Material Design 3全項目の適用性台帳、XR向けM3設計、録画専用semantic color、非drag操作、言語非依存UI、日英README、アイコン資産の固定・検証・オフライン配布を追加。coverage／mutationの数値閾値をrelease条件から外し、品質傾向を観測する参考指標へ変更
...
10. t-wada式のRed–Green–Refactorを採用し、要求された振る舞いと外部境界を小さいテストで固定する。coverage／mutationは傾向把握の参考指標として収集するが、数値閾値をmerge／release条件にはしない。
...
| NFR-010 | テスト | 必須unit／contract／integration testが成功すること。line／branch coverageとmutation scoreは参考指標として記録し、数値だけでmerge／releaseを拒否しない |
...
- 初期partと変更後partはH.264 4:2:0で厳密一致できる偶数寸法だけを受理し、奇数寸法を暗黙にpadした別寸法のpartは作らない。
...
- coverage／mutationの数値を上げるためだけのテストを作らない。実際の要求、回帰、外部境界に対応するテストだけを追加する。
...
Public native ABI contract scenarios       = pass
Managed line／branch coverage               = report per major assembly
Native line／branch coverage                = report
Mutation score                              = optional diagnostic report
```

coverage／mutationには最低合格率を設けず、低下だけを理由にCI、merge、releaseを失敗させない。傾向を比較できるよう対象と除外は明示し、変更時はレビューする。managed metricsの対象は`VRRecorder.Application`、`VRRecorder.Compliance`、`VRRecorder.Domain`、`VRRecorder.Infrastructure.Media`、`VRRecorder.Infrastructure.Osc`、`VRRecorder.Infrastructure.SteamVr`、`VRRecorder.Infrastructure.Storage`、`VRRecorder.Presentation.Wrist`の8 packageとする。`eng/managed-coverage.runsettings`で収集したCoberturaを`VRRecorder.ManagedCoverage`へ渡し、assembly別のline／branch値を表示する。report欠落／破損は測定系の異常として明示するが、coverage値そのものは合否判定へ使わない。
...
6. coverage metricsの収集・表示
7. mutation testは必要な調査時だけ差分対象を参考計測
...
- integration coverage trend report
- 必要な範囲のmutation diagnostic
...
| coverage数値の目的化 | 実装より数値合わせを優先 | coverage／mutationは参考値に限定し、要求・回帰・公開契約に結び付くテストだけを追加 |
