...
```text
dotnet test VR-Recorder.sln --no-restore --verbosity minimal
dotnet build src/VRRecorder.App/VRRecorder.App.csproj --no-restore --verbosity minimal
dotnet format VR-Recorder.sln --no-restore --verify-no-changes --verbosity minimal
make -C tests/VRRecorder.Native.Tests test
```

- managed: 827件成功、失敗0、skip 0
  - Domain 90
  - Application 223
  - Compliance 170
  - Presentation 84
...
設計書18.5に従い、`VRRecorder.IntegrationTests`だけを実行してCoberturaを収集した直前checkpointの値です。今回追加したruntime Mic／Mute境界を含む827件の回帰は成功していますが、coverageは追加後に再収集していないため、次の表を現在値として扱いません。
...
- managed: 827 passed, 0 failed, 0 skipped
  - Domain 90
  - Application 223
  - Compliance 170
  - Presentation 84
...
Following design section 18.5, Cobertura coverage was collected by running only `VRRecorder.IntegrationTests` at the preceding checkpoint. The 827-test regression including the new runtime Mic/Mute boundary passes, but coverage has not been recollected after those additions; the following table is not a current measurement.
