...
- 同じ公式tarballから再現生成した隔離FFmpeg 8.1.2 SDKでの厳格build／実libavcodec／libswresample／libavformat CTest: 46/46成功。実AAC packet encoder、実libavformat Port、実AAC descriptor→実MOV header結合testはASan／UBSan＋leak検出とGCC TSanで各10回反復成功し、packet encoderと結合testは通常構成でも各100回反復成功
...
- production audio pumpの入力windowを1024 frameへ固定してnative AAC factoryへ接続し、full worker callbackからpeer Joinするcross-stream再入境界を解消すること。H.264 encoder Port、factory／compositionを加え、実libavcodec／libavformat Portをversion／source commit固定かつ法務承認済みdependencyとしてproduction media backendへ接続すること。現状のC ABI backendは引き続き`BACKEND_UNAVAILABLE`
