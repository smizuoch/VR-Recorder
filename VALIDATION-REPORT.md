...
- 同じ公式tarballから再現生成した隔離FFmpeg 8.1.2 SDKでの厳格build／実libavcodec／libswresample／libavformat CTest: 45/45成功。実AAC packet encoderと実libavformat PortはいずれもASan／UBSanとleak検出下で10回反復成功
...
- 公式FFmpeg 8.1.2のnative AAC context／ASCを生成し、packed Float32 stereoをlibswresampleでFLTPへ変換、FIFOで任意chunkを1024 sample frameへ再構成し、Finish時だけsmall-lastとNULL drainを行うpacket encoder。0／1／1023／1024／1025 sampleのexact timing、全factory／runtime失敗、途中batch atomicity、microsecond overflow、Encode／Finish／Abort競合を実APIで検証する
...
- 実AAC packet encoderの入力window上限固定または内部chunk化、exact 192 kbpsのdescriptor／MOV codec-parameter伝播、full worker callbackからpeer Joinするcross-stream再入境界を決定し、H.264 encoder Port、factory／compositionを加えて実libavformat Portとともにversion／source commit固定かつ法務承認済みdependencyとしてproduction media backendへ接続すること。現状のC ABI backendは引き続き`BACKEND_UNAVAILABLE`
...
- Strict build/real-libavcodec/libswresample/libavformat CTest against an isolated FFmpeg 8.1.2 SDK reproducibly built from the same official tarball: 45/45 passed; the real AAC packet encoder and libavformat port each passed 10 repeated runs under ASan/UBSan with leak detection
...
- A real-API packet encoder that creates the official-FFmpeg-8.1.2 native AAC context/ASC, converts packed stereo Float32 into FLTP with libswresample, rebuilds arbitrary chunks into 1024-sample frames through a FIFO, and sends small-last plus a NULL drain only during Finish; tests pin exact timing for 0/1/1023/1024/1025 samples, every factory/runtime failure, mid-batch atomicity, microsecond overflow, and Encode/Finish/Abort races
...
- Fix an input-window cap or internal chunking for the real AAC packet encoder, propagate the exact 192 kbps descriptor into MOV codec parameters, close the full-worker cross-stream callback/peer-Join boundary, add an H.264 encoder port and factory/composition wiring around the real libavcodec/libavformat ports, then admit the version/source-commit-pinned dependency through legal approval and connect it to the production media backend; the C ABI backend still returns `BACKEND_UNAVAILABLE`
