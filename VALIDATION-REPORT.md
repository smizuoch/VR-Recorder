...
- callback内ではsame-session Abortだけを許し、request_stop／destroyを含む他session APIをheader契約で禁止するrecursive callback quiescence、stop workerの自己join回避、Start成功後だけpending first-packet milestoneをcommitして失敗Startでは破棄するC ABI／backend lifetime境界
