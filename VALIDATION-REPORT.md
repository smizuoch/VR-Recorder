...
- 実AAC packet encoderの入力window上限固定または内部chunk化と共通mux submission seamを決定し、H.264 encoder Port、factory／compositionを加えて実libavformat Portとともにversion／source commit固定かつ法務承認済みdependencyとしてproduction media backendへ接続すること。現状のC ABI backendは引き続き`BACKEND_UNAVAILABLE`
...
- Fix an input-window cap or internal chunking for the real AAC packet encoder, choose the common mux-submission seam, add an H.264 encoder port and factory/composition wiring around the real libavcodec/libavformat ports, then admit the version/source-commit-pinned dependency through legal approval and connect it to the production media backend; the C ABI backend still returns `BACKEND_UNAVAILABLE`
