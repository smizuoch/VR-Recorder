...
- 初回setupの現在項目を12個の個別localized resource、1-origin step番号、全体件数、進捗率へ投影する英日／200% pseudo／high-contrast／accessible WPF画面。録画権利認可より先に未完了を表示し、閉じても録画を無効のまま保ち、画面自身には完了APIを持たせない（SteamVR以外のprobe接続は未実装）
- 現在の初回setup項目をprobeへ渡し、成功時だけ順序付き進捗をatomic保存するverification経路。SteamVR App ID 250820のWindows Steam登録を3 registry viewで照合し、`Installed=1`だけで先頭項目を進めるWPF操作を構成済み（Windows実行と残り11 probeは未検証／未実装）
...
- 初回setupの残り11 probeとWindows上のSteamVR検出実行、VR配置／OSC設定のruntime反映、実アプリのend-to-end録画
...
- managed: 899 passed, 0 failed, 0 skipped
  - Domain 90
  - Application 242
  - Compliance 186
  - Presentation 89
...
- An English/Japanese, deterministic 200%-pseudo, high-contrast, accessible WPF window that projects the current first-run item, one-based step number, total count, and overall progress; it appears before rights authorization, keeps recording disabled after close while incomplete, and exposes no self-approval API (probe wiring beyond SteamVR remains outstanding)
- A verification path that sends only the current first-run item to a probe and atomically saves ordered progress only after success; the composed WPF action advances the first item only for `Installed=1` from SteamVR App ID 250820 across three Windows registry views (Windows execution and the remaining 11 probes are unverified/outstanding)
...
- The remaining 11 first-run probes and Windows execution of SteamVR detection, runtime VR-placement/OSC settings, and end-to-end recording in the real application
