...
desktop production compositionはP/Invoke Spout source、encoder probe、native recording engine、OSC、storage、Legal mirror、runtime fault stop、SteamVR inputまで配線済みです。stale CameraLease／未確定録画の起動時回復、複数VRChatの厳密選択、録画設定UI、4状態tray、保存path／カメラ復元警告、privacy-safe診断bundleの明示exportも配線済みです。ただしproduction native media／SteamVR backend自体は意図的に`BACKEND_UNAVAILABLE`を返します。承認済みWindows x64 native DLLとffprobeもRelease入力として未提供です。したがって、現状は設計契約と境界実装を検証する開発checkpointであり、録画可能な製品ではありません。
...
設計書18.5に従い、`VRRecorder.IntegrationTests`だけを実行してCoberturaを収集した直前checkpointの値です。今回追加した通知、設定、tray、診断exportを含む778件の回帰は成功していますが、coverageは追加後に再収集していないため、次の表を現在値として扱いません。
...
- app／OS／GPU／encoder、frame統計、A/V sync、audio、OSC、finalization失敗を網羅する構造化診断event
...
- managed: 778 passed, 0 failed, 0 skipped
  - Domain 90
  - Application 193
  - Compliance 170
  - Presentation 77
...
Following design section 18.5, Cobertura coverage was collected by running only `VRRecorder.IntegrationTests` at the preceding checkpoint. The 778-test regression including the new notifications, settings, tray, and diagnostics export passes, but coverage has not been recollected after those additions; the following table is not a current measurement.
...
- Complete structured diagnostics for app/OS/GPU/encoder, frame statistics, A/V sync, audio, OSC, and finalization failures
